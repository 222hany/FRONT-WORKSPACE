...
/*
    displayImg(page) : page는 현재 페이지 번호를 나타냄. 호출될 때마다 페이지를 전환하고 해당 페이지의 이미지를 표시하는데 사용.
...
function updateButtons(){
    ...
    /*
        다음 버튼에서 마지막 페이지일 경우 이미지가 없을 때 더 이상 다음 버튼이 눌리는 것을 방지하기 위해 다음 버튼 또한 클릭하지 못하게 막음.
        (currentPage + 1) * imagesPerpage : 다음 페이지에 해당하는 이미지의 첫 번째 인덱스를 나타냄.
                            images.length : images 배열의 길이로, 페이지를 넘어갈 수 있는 이미지의 전체 수를 나타냄.
        (currentPage + 1) * imagesPerpage >= images.length : 다음 페이지에 해당하는 이미지가 이미지 배열을 벗어나는지 확인 후 다음 페이지에 이미지가 더 이상 없을 때 다음 버튼을 비활성화.
    */
